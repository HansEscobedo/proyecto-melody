@extends('app')

@section('title')
    Dashboard {{ auth()->user()->name }}
@endsection

@section('content')
    <div class="p-6 bg-gray-200 rounded-lg shadow-lg md:flex-col md:justify-center ">
        <h2 class="p-6 text-3xl font-bold text-center text-black uppercase">Selecciona una opción</h2>
        <div class="md:flex md:justify-evenly">
            <div>
                <a href="{{ route('concert.list') }}"
                    class="p-3 font-bold text-center text-white transition rounded bg-blue-custom-700 hover:bg-blue-custom-1000">Ver conciertos</a>
            </div>

            <div>
                <a href="{{ route('client.concerts') }}"
                class="p-3 font-bold text-center text-white bg-blue-700 rounded">Detalle de compras realizadas</a>
            </div>
        </div>
    </div>
@endsection
